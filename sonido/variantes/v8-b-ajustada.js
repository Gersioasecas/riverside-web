...
function construirMar(ctx, dur) {
  ...
  /* ── LA OLA, tal como la describió Sergio oyendo la v4 ──────────────────
       «en cada ola es un sonido como de burbujeo que viene del silencio muy
        suave, y sube hasta que rompe… cuando la ola se pierde en la parte
        final… suena justo como cuando muchas burbujitas de alguna pastilla
        efervescente se revientan… ese momento de retroceso, cuando el mismo
        sonido se va haciendo de nuevo tenue hacia atrás, tiene que durar casi
        lo mismo que el de entrada»

     De ahí salen estos tres números. En v4 el ataque era de 0.12 s: la ola
     aparecía de golpe. Ahora tarda 1.7 s en llegar arriba, se queda 0.55 s
     reventando, y se retira con una constante de 1.55 s — que da una cola de
     duración comparable a la subida, como pidió. */
  const ATAQUE = 1.70, SOSTEN = 0.55, TAU = 1.55, VAR_EVT = 0.22;
  const CURVA_SUBIDA = 1.75;       // >1 arranca del silencio y acelera al romper
  const KERNEL_S = 1.4;            // cola audible de la línea de rompiente
  const N_EVT = 5, EVT_S = 9.0;
  const LECHO_S = 28, LECHO_VEL = [1, 1.07, 0.94], LECHO_PAN = [-0.95, 0.95, 0.81];

  /* ── EL RITMO ───────────────────────────────────────────────────────────
       «de momento suena como un rush de ola tras ola, y eso no refleja
        tranquilidad, tiene que ser sí aleatorio pero en un rango que lo
        mantenga casi constante, como cuando el mar está tranquilo, cada ola
        rompe cada 4 o 5 segundos»

     v4 tenía SEIS trenes con periodos de 6.7 a 29.3 s. Sumando sus tasas
     salía una ola cada 2.05 s, y con VAIVEN = 0.75 el intervalo bailaba entre
     el 25 % y el 175 % del periodo: a veces caían casi encima. Eso era el rush.

     Ahora un solo tren a 4.6 s con VAIVEN 0.13, o sea intervalos de 4.0 a 5.2 s.
     Aleatorio, pero dentro de un rango estrecho. Cada ola sigue viniendo de un
     sitio distinto del campo estéreo (OLA_PAN), que es lo que da la sensación
     de varias olas sin necesidad de amontonarlas.

     Con olas de ~9 s e intervalos de 4.6 s, cada una empieza cuando la
     anterior lleva la mitad: el mar nunca se queda en silencio, que es
     justo lo que él observó. */
  const TRENES = [[4.6, 1.00, 0]];
  const VAIVEN = 0.13, OLA_GMIN = 0.82, OLA_GSPAN = 0.18, OLA_PAN = 0.85;

  const CORTE_PENACHO = [110, 0.6];
  const AIRE = [['lowpass', 6033, 0.5, 0], ['highshelf', 13420, 0.7, -23.5],
                ['highshelf', 643, 0.52, -1.6]];
  const TECHO = [0.75, 0.98];
  /* «me gustaría que suene más lejos, o sea el volumen por defecto más bajo,
      si lo actual es 100% ponlo al 50%» — v4 estaba en 2.2. */
  const MAESTRO = 1.1;
  ...
  function envolvente(at, so, ta) {
    ...
    for (let i = 0; i < ns; i++) {
      const t = i * PASO_E;
      /* La subida ya no es un exponencial saturante (que arranca de golpe y
         se aplana), sino una potencia: sale del silencio muy despacio y
         acelera hacia el momento de romper. Es lo que se oye en la playa. */
      const sube = t < at ? Math.pow(t / at, CURVA_SUBIDA) : 1;
      src[i] = sube * (t < at + so ? 1 : Math.exp(-(t - at - so) / ta));
    }
