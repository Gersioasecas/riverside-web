...
function construirMar(ctx, dur) {
  ...
  const TILT      = 8;    // dB/oct que quita la cascada de highshelf
  const FCORTE    = 180;    // Hz del highpass
  const RETUMBO   = 0;      // dB de realce del grave (lowshelf)
  const RETUMBO_F = 170;    // Hz de ese realce
  const FRODILLA  = 1800;   // Hz donde empieza la caída de agudos
  const NHP       = 2;
  const MAESTRO   = 1.4912;
  const NVOCES    = 21;
