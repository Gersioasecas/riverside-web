...
function construirMar(ctx, dur) {
  ...
  const NHP       = 2;
  const MAESTRO   = 2.1795;
  const NVOCES    = 21;
  const PROF      = 0.8;   // 0 = mar plano · 1 = la voz cae a silencio entre olas
  const INTERVALO = 14;     // s entre olas de UNA voz
  const TA        = 1.2;    // s de subida de una ola
  const TD        = 3;    // s de bajada
  const TH        = 6;      // s de MESETA entre subida y bajada
  const GAMMA     = 1;    // <1 comprime las crestas de la envolvente
