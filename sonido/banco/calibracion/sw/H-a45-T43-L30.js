...
function construirMar(ctx, dur) {
  ...
  const MAESTRO   = 2.0115;
  const NVOCES    = 19;
  const NTRENES   = 3;
  const T0        = 4.3;    // s entre olas
  const REPARTO   = 1.22;
  const ANCHO_A   = 0.45;   // borde agudo de la banda, en periodos de ola
  const ANCHO_B   = 1.15;   // borde grave
  const SESGO     = 0;   // asimetría de la ola
