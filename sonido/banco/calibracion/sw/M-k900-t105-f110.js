...
function construirMar(ctx, dur) {
  ...
  const TILT      = 10.5;    // dB/oct que quita la cascada de highshelf
  const FCORTE    = 110;    // Hz del highpass
  const RETUMBO   = 0;      // dB de realce del grave (lowshelf)
  const RETUMBO_F = 170;    // Hz de ese realce
  const FRODILLA  = 900;   // Hz donde empieza la caída de agudos
  const NHP       = 2;
  const MAESTRO   = 2.2467;
  const NVOCES    = 21;
  const PROF      = 0.001;   // 0 = mar plano · 1 = la voz cae a silencio entre olas
  const INTERVALO = 34;     // s entre olas de UNA voz
  const TA        = 3.4;    // s de subida de una ola
  const TD        = 7.6;    // s de bajada
  const TH        = 0;      // s de MESETA entre subida y bajada
  const GAMMA     = 1.0;    // <1 comprime las crestas de la envolvente
  ...
  const NE = 4096;
  function ondas(periodoS, intervalo, ta, td, th, gam) {
    th = th || 0; gam = gam || 1;
    ...
    for (let t = Math.random() * intervalo; t < periodoS; t += intervalo * (0.55 + 0.9 * Math.random())) {
      ...
      const h = th * (0.7 + 0.6 * Math.random());
      for (let i = Math.round((t - a) * mps); i <= Math.round((t + h + c) * mps); i++) {
        const dt = i / mps - t;
        const v = dt < 0 ? 0.5 - 0.5 * Math.cos(Math.PI * (dt + a) / a)
                : dt < h ? 1
                         : 0.5 + 0.5 * Math.cos(Math.PI * (dt - h) / c);
        d[((i % NE) + NE) % NE] += A * v;
      }
    }
    let s = 0; for (let i = 0; i < NE; i++) s += d[i];
    for (let i = 0; i < NE; i++) d[i] = Math.pow(d[i] * NE / (s + 1e-9), gam);
    s = 0; for (let i = 0; i < NE; i++) s += d[i];
  ...
  for (let k = 0; k < NVOCES; k++) {
    ...
    const env = lento(ondas(per, INTERVALO * (0.75 + 0.5 * ((k * 0.4531) % 1)), TA, TD, TH, GAMMA), per);
