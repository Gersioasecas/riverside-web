...
function construirMar(ctx, dur) {
  ...
  const MAESTRO   = 1.9898;
  const NVOCES    = 19;
  const NTRENES   = 3;
  const T0        = 5.5;    // s entre olas
  const REPARTO   = 1.22;
  const JITTER    = 0.30;
  const AMPJIT    = 0.40;   // dispersión relativa del tamaño de ola
  const DERIVA    = 26;     // s · por encima de esto el mar no cambia de nivel
  const ESTABLE   = 2;    // s · ventana con que se aplana la deriva 1/f del ruido
  ...
  const CAIDA     = 0.62;
  const PROF      = 0.001;
  const VIENTO_dB = 0.001;    // L10-L90 del viento
  const VIENTO_T  = 407;
  ...
  function tren(periodoS, t0) {
    ...
    while (t < periodoS) {
      const A0 = 1 + AMPJIT * (Math.random() + Math.random() - 1) * 1.73;
      let tb = t;
    ...
    /* fuera la deriva lenta: la intensidad media de una rompiente es estable
       en la escala del minuto; lo que se mueve es la ola, no el mar entero. */
    if (DERIVA > 0 && DERIVA < periodoS / 2) {
      const W = Math.max(3, Math.round(DERIVA * mps));
      const suave = new Float32Array(NE);
      let acc = 0;
      for (let i = -W; i < 0; i++) acc += d[((i % NE) + NE) % NE];
      for (let i = 0; i < NE; i++) {
        acc += d[i] - d[(((i - W) % NE) + NE) % NE];
        suave[i] = acc / W;
      }
      let m0 = 0; for (let i = 0; i < NE; i++) m0 += d[i];
      m0 /= NE;
      for (let i = 0; i < NE; i++) d[i] = Math.max(0, m0 + (d[i] - suave[i]));
    }
