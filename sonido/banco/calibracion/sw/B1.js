...
function construirMar(ctx, dur) {
  ...
  const MAESTRO   = 1.4837;
  const NVOCES    = 21;     // sectores de costa
  const NTRENES   = 3;      // trenes de oleaje independientes
  const T0        = 4.1;    // s entre olas
  const REPARTO   = 1.22;   // cuánto se separan los periodos de los tres trenes
  const JITTER    = 0.32;   // dispersión relativa del periodo
  const PEEL      = 4;    // s que tarda la rotura en barrer el arco audible
  const TA        = 1.1;    // s de subida por sector
  const TD        = 2.7;    // s de bajada (el lavado de espuma)
  const PROF      = 0.5;   // profundidad de la modulación por sector
  const BARRAS    = 1;      // líneas de rotura que cruza cada ola
  const DBARRA    = 2.2;    // s entre una barra y la siguiente
  const CAIDA     = 0.62;   // cuánto se debilita la rotura en cada barra
  const VIENTO_dB = 1.5;
