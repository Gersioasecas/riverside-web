...
function construirMar(ctx, dur) {
  ...
  const T0        = 7;    // s entre olas
  const REPARTO   = 1.22;   // cuánto se separan los periodos de los tres trenes
  const JITTER    = 0.32;   // dispersión relativa del periodo
  const PEEL      = 5.5;    // s que tarda la rotura en barrer el arco audible
  const TA        = 1.1;    // s de subida por sector
  const TD        = 2;    // s de bajada (el lavado de espuma)
  const PROF      = 0.55;   // profundidad de la modulación por sector
  const BARRAS    = 3;      // líneas de rotura que cruza cada ola
  const DBARRA    = 1.9;    // s entre una barra y la siguiente
