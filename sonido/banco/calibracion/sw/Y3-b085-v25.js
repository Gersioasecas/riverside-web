...
function construirMar(ctx, dur) {
  ...
  const MAESTRO   = 2.2429;
  const NVOCES    = 19;
  const NTRENES   = 1;
  const T0        = 5.2;    // s entre olas
  const REPARTO   = 1.22;
  const ANCHO_A   = 0.38;   // borde agudo de la banda, en periodos de ola
  const ANCHO_B   = 0.85;   // borde grave
  ...
  const PROF      = 0.28;
  const VIENTO_dB = 0.25;    // L10-L90 del viento
  const VIENTO_T  = 407;
