...
function construirMar(ctx, dur) {
  ...
  const TILT      = 6;    // dB/oct que quita la cascada de highshelf
  const FCORTE    = 130;    // Hz del highpass
  const RETUMBO   = 8;      // dB de realce del grave (lowshelf)
  const RETUMBO_F = 160;    // Hz de ese realce
  const NHP       = 2;
  const MAESTRO   = 1.4761;
  const NVOCES    = 21;
  const PROF      = 0.75;   // 0 = mar plano · 1 = la voz cae a silencio entre olas
