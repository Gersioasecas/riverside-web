...
function construirMar(ctx, dur) {
  ...
  const TILT      = 6.8;    // dB/oct que quita la cascada de highshelf
  const FCORTE    = 150;    // Hz del highpass
  const RETUMBO   = 12;      // dB de realce del grave (lowshelf)
  const RETUMBO_F = 160;    // Hz de ese realce
  const NHP       = 2;
  const MAESTRO   = 2.1519;
  const NVOCES    = 21;
  const PROF      = 0.95;   // 0 = mar plano · 1 = la voz cae a silencio entre olas
  const INTERVALO = 19;     // s entre olas de UNA voz
  const TA        = 1.6;    // s de subida de una ola
  const TD        = 4;    // s de bajada
  const TH        = 10;      // s de MESETA entre subida y bajada
  const GAMMA     = 0.5;    // <1 comprime las crestas de la envolvente
