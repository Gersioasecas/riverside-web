...
function construirMar(ctx, dur) {
  ...
  const RETUMBO   = 14;      // dB de realce del grave (lowshelf)
  const RETUMBO_F = 160;    // Hz de ese realce
  const NHP       = 2;
  const MAESTRO   = 0.9798;
  const NVOCES    = 21;
  const PROF      = 0.95;   // 0 = mar plano · 1 = la voz cae a silencio entre olas
  const INTERVALO = 16;     // s entre olas de UNA voz
  const TA        = 2.6;    // s de subida de una ola
  const TD        = 6;    // s de bajada
