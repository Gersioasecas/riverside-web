...
function construirMar(ctx, dur) {
  ...
  const MAESTRO  = 1.8298;    // medido: deja el pico en ~0.80
  ...
  const APLANA   = 2.0;     // iguala la altura de los crestones (función IMPAR)
  ...
  /* ── el viento ────────────────────────────────────────────────────────── */
  const VIENTO_dB  = 0.9;   // L10−L90. Más de ~¼ de la ola y `olas/m` se hunde
  const VIENTO_T   = 900;   // s
